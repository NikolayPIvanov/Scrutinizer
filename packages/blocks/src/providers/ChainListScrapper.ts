...
import { TYPES, ILogger, IScrapper, IConfiguration } from "../types";
import { to } from "../utils";

const DEFI_LLAMA_GITHUB_BASE_URL = "https://raw.githubusercontent.com/DefiLlama/chainlist/";

const MAIN_GITHUB_RPC_URL = "main/constants/chainIds.json";
const FALLBACK_GITHUB_RPC_URL = "5e2fa1ff401f5e66ebd02b3828da7180c1a7e2f3/constants/chainIds.json";

const MAIN_RPC_GITHUB_RPC_URL = "main/constants/extraRpcs.js";
const FALLBACK_RPC_GITHUB_RPC_URL = "b9857beb81d48c041ad906a6bb51e4b36fc58186/constants/extraRpcs.js";
...
export class ChainListScrapper implements IScrapper<any> {
    ...
    public scrape = async () => {
        const [chainIdsList, rpcUrlsList] = await Promise.all([this.scrapeChainIds(), this.scrapeRpcUrls()]);
        if (!rpcUrlsList) {
            throw new Error("No RPC list variable!")
        }

        const chainRpcUrlListMapping = Object.keys(rpcUrlsList).reduce((acc, key) => {
            return {
                ...acc,
                [key]: {
                    ...rpcUrlsList[key],
        ...
        const data = Object.values(chainRpcUrlListMapping)
            ...
            .filter((x: any) => x.name && x.rpcs.length >= 3 && x.chainId === this.configuration.network.chainId);
