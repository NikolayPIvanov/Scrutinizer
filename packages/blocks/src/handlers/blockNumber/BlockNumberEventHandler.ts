...
import { IBlockJob, IBlockNumberEmitter, IBlockNumberEventHandler, IConfiguration, IEmitter, IKafkaClient, ILogger, IProvider, TYPES } from '../../types';
...
export class BlockNumberEventHandler implements IBlockNumberEventHandler {
    constructor(@inject(TYPES.ILogger) private logger: ILogger,
        ...
        @inject(TYPES.IBlockNumberEmitter) private blockNumberEmitter: IBlockNumberEmitter,
    ...
    private bootstrap = () => {
        this.emitter.instance.on(constants.events.newBlock,
            async ({ block, retries = 0, callback }: IBlockJob) => this.handle({ block, retries, callback }))
    }

    private handle = async (job: IBlockJob) => {
        this.logger.info(`Received Block ${parseInt(job.block.number, 16)}`);
